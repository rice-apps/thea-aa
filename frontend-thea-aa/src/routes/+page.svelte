...
<main class="flex h-full p-10 space-x-9">
	<aside class="w-1/3 bg-white rounded-lg shadow-lg p-8">
		...
		<div class = "overflow-x-auto whitespace-nowrap space-x-2 flex py-2">
			<Button on:click={() => currentView = 'superfund' } class="flex flex-col items-center">
				<Building></Building>
				<span class="text-xs">Superfund</span>
			</Button>
			<Button on:click={() => currentView = 'emission'} class="flex flex-col items-center">
				<Cloudy></Cloudy>
				<span class="text-xs">Emission Events</span>
			</Button>
			<Button on:click={() => currentView = 'air quality'} class="flex flex-col items-center ">
				<Wind></Wind>
